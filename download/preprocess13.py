...
def get_input():
    ...
    with open(r'download/single-level.nc', 'rb') as f:
...
# Dimensions:    698MB                   (batch: 1, time: 2, lat: 721, lon: 1440,
#                                    level: 13)
# Coordinates:
...
#     temperature                   (batch, time, level, lat, lon) float32 108MB ...
...
def get_targets(timesteps = 40):
    lat = np.linspace(-90, 90, 721, dtype=np.float32)
    lon = np.linspace(0, 359.75, 1440, dtype=np.float32)
    level = np.array([50, 100, 150, 200, 250, 300, 400, 500, 600, 700, 850, 925, 1000], dtype=np.int32)
    ...
    shape_3d = (1, timesteps, 13, 721, 1440)
    ...
    return targets
# Dimensions:      1GB            (batch: 1, time: 4, lat: 721, lon: 1440, level: 13)
# Coordinates:
#   * lon                      (lon) float32 6kB 0.0 0.25 0.5 ... 359.5 359.8
...
# Dimensions without coordinates: batch
# Data variables:
#     2m_temperature           (batch, time, lat, lon) float32 17MB 248.3 ... 2...
#     mean_sea_level_pressure  (batch, time, lat, lon) float32 17MB 9.983e+04 ....
#     10m_v_component_of_wind  (batch, time, lat, lon) float32 17MB -0.04184 .....
#     10m_u_component_of_wind  (batch, time, lat, lon) float32 17MB 0.04857 ......
#     total_precipitation_6hr  (batch, time, lat, lon) float32 17MB 7.443e-05 ....
#     temperature              (batch, time, level, lat, lon) float32 216MB 238...
#     geopotential             (batch, time, level, lat, lon) float32 216MB 1.9...
#     u_component_of_wind      (batch, time, level, lat, lon) float32 216MB 0.0...
#     v_component_of_wind      (batch, time, level, lat, lon) float32 216MB 0.0...
#     vertical_velocity        (batch, time, level, lat, lon) float32 216MB 0.0...
#     specific_humidity        (batch, time, level, lat, lon) float32 216MB 2.8... <xarray.Dataset> Size: 17MB

def get_forcings(year, month, day, timesteps = 40):
...
# Dimensions:                       (batch: 1, time: 4, lat: 721, lon: 1440)
# Coordinates:
#   * lon                           (lon) float32 6kB 0.0 0.25 0.5 ... 359.5 359.8
#   * lat                           (lat) float32 3kB -90.0 -89.75 ... 89.75 90.0
#   * time                          (time) timedelta64[ns] 32B 06:00:00 ... 1 d...
# Dimensions without coordinates: batch
# Data variables:
#     toa_incident_solar_radiation  (batch, time, lat, lon) float32 17MB 1.978e...
#     year_progress_sin             (batch, time) float32 16B 0.01559 ... 0.02849
#     year_progress_cos             (batch, time) float32 16B 0.9999 ... 0.9996
#     day_progress_sin              (batch, time, lon) float32 23kB -8.742e-08 ...
#     day_progress_cos              (batch, time, lon) float32 23kB -1.0 ... 0....
